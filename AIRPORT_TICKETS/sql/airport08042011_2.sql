...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `flights_view` AS 
  select 
    ...
    concat('Рейс, код - ',`fl`.`flight_code`,', компания ',`cmp`.`company_name`,', отправка из ',`src`.`point_name`,', прибытие в ',`dst`.`point_name`) AS `flight_name` 
  from 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `shedules_view` AS 
  select 
    ...
    concat('Транспорт: ',`atp`.`type_name`,', вылет: ',`shd`.`source_dt`,', прибытие: ',`shd`.`dest_dt`,', позиция в расписании ',`flv`.`flight_name`) AS `shedule_name` 
  from 
...
INSERT INTO `users` (`id`, `username`, `password`, `email`, `isactive`, `person_id`, `closed`, `enable_admin`, `enable_deleting`) VALUES 
  (7,'operator1','c2VpZjUzNm1hc2hh','psdevelop@yandex.ru',1,55,0,'0','1'),
