...
require_once("classes/object_collection.class.php");
require_once("classes/object_adapters/airpoint_object_adapter.class.php");
require_once("classes/object_adapters/airtype_object_adapter.class.php");
require_once("classes/object_adapters/company_object_adapter.class.php");
require_once("classes/object_adapters/flight_object_adapter.class.php");
require_once("classes/object_adapters/shedule_object_adapter.class.php");
require_once("classes/object_adapters/ticket_object_adapter.class.php");
require_once("classes/object_adapters/country_object_adapter.class.php");
...
abstract class TableAdapter extends Tools {
    ...
    function selectWithRelative()   {
        ...
        $rows=$this->dbconnector->query_both_to_array(
            "SELECT * FROM ".$this->with_relative_view_name." WHERE ( ".$this->base_filter." OR ".
				...
				" order by id desc limit {$start_limit},{$end_limit};");
        //echo (
    ...
    function selectFullWithRelative()   {
        $rows=$this->dbconnector->query_both_to_array(
            "SELECT * FROM ".$this->with_relative_view_name." WHERE ".$this->base_filter." AND (closed<>1) AND ".
                $this->getFilterWhereClause()." order by id desc;");
        if ($rows!=null)    {
            $this->all_capacity = sizeof($rows);
            $this->select_collection = new ObjectCollection($this->class_name,$rows);
        }   else    {
    ...
    function generateDictHeader()   {
        echo "<div class=\"dict_header_default\">Таблица: <b><span style=\"font-size:14px;\">{$this->dict_header}</span></b>. ".
                " Часть: ".($this->current_part_num+1)." [".
                        ...
                        (($this->part_capacity*$this->current_part_num)+$this->all_capacity)."] <input type=\"hidden\" 
    ...
    function getSelectArray()   {
        $select_rows=$this->dbconnector->query_both_to_array(
            "SELECT id, ".$this->object_adapter->select_display_field.
                " as select_name FROM ".$this->with_relative_view_name." WHERE ".$this->base_filter." AND (closed<>1);");
